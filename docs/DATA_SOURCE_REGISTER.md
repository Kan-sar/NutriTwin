...
| USDA-FDC | USDA FoodData Central API/downloads | https://fdc.nal.usda.gov/api-guide/; releases vary | CC0 1.0; attribution requested | Optional demo/gap foods: FDC ID, description, data type, nutrient IDs/amounts/units; scripted API/import | Not Indian-authoritative; food matching and preparation differences must be visible |
...
The initial target and food fixtures, if synthetic, use source IDs prefixed `DEMO-SYNTHETIC`, conspicuous descriptions, and `authoritative=false`. They exercise software only and cannot validate nutrition science. FDC-derived fixtures use fixed FDC identifiers and store the CC0 source manifest.
